...
| Telegram 消息名称 | 给每台 VPS 填不同名称；直接回车默认使用 Hy2 域名 |
...
所有“是/否”提示都会直接写明回车默认值，例如：

```text
开启原生端口跳跃？[Y/n，直接回车默认：是]:
是否现在开启 Telegram 成功连接提醒？[y/N，直接回车默认：否]:
```
...
因此，不能只看“一键脚本名字”判断谁更快。若另一个脚本使用同一版 Hysteria，真正差异通常来自线路、VPS CPU/网卡、客户端核心和拥塞控制。准确填写且不超过真实链路能力的 Brutal 在部分高丢包线路上可能获得更激进的短时吞吐；填得过高则可能更慢、更抖并浪费流量。当前脚本选择自适应 BBR 的保守配置，目标是不同设备、不同网络下更稳，而不是保证每次测速数字最大。
...
默认模式确实存在网站伪装：它通过 Hysteria 官方 `string` 模式返回本机固定小页面，不是“完全没有伪装”。未认证探测者看到的是普通 HTTP/3 页面，而不是 Hy2 密码或客户端数据。

使用 Bing 不等于自动更安全。反代大站能提供更丰富的网页内容，但大站内容出现在无关域名下也不一定更自然，而且任何未认证访客都可能触发 VPS 请求上游并消耗流量。默认本机页面更省流量、依赖更少；如果你担心许多服务器使用相同固定页面形成共同特征，更好的做法是反代一个你自己控制、内容正常且体积较小的 HTTPS 网站，而不是认为“Bing”三个字本身可以隐藏 IP 或保证不被识别。

Hysteria 协议本身会把已认证连接伪装成正常 HTTP/3 加密流量；网站伪装主要用于应对未认证 HTTP/3 请求和主动探测。任何方案都不能承诺无法被流量分析，也不会隐藏直接连接所需的 VPS IP、域名或 UDP 端口。
...
8. 输入这台 VPS 的 Telegram 消息名称，例如 `洛杉矶-01`、`日本家宽`；直接回车默认使用 Hy2 域名。
9. 脚本发送测试消息后，确认自己的 Telegram 确实收到，再在终端输入大写 `YES`；未确认时不会保存配置。
...
所有连接提醒、重连汇总、日报、月报、手动报告和测试消息都会显示这台 VPS 的消息名称。同一个 Bot 和 Chat ID 可以供多台 VPS 使用，只要为每台设置不同名称即可。

以后修改名称，选择菜单 `11) 设置 Telegram 消息名称`，或者：
...
日报只统计报告日期当天的流量，不是从安装日起不断累计。月报只合计对应自然月的日报；“本月累计”也是从当月 1 日开始相加，进入下个月后新月份从 `0` 开始，旧月份不会混入。脚本不粗暴清空 Hysteria 或网卡的底层总计数器，而是把相邻采样的差值归入对应日期，因此服务重启和跨月都不会重复计算历史总量。

日报按北京时间自然日归档，但它是每分钟采样形成的运维统计，不是逐包计费账本；跨越零点的最多一个采样间隔可能落在相邻一天。月度趋势不受明显影响，套餐结算仍应以云厂商面板为准。状态文件最多保留约 70 天的日记录，足够生成当前月和上一个月报告，避免长期运行后无限增长。
